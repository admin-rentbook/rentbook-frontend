...
export const PropertyListMobile = ({ row }: PropertyListMobileProps) => {
  const { bgColor, textColor, fillColor } = returnStatus(row.original.status);
  return (
    <div className="flex flex-col gap-1 border-b border-b-sidebar-border pb-2">
      <div className="flex justify-between">
        <h5 className="text-body text-black-500">{row.original.name}</h5>
        <ArrowRight01Icon />
      </div>
      <p className="text-body-small text-black-400">{row.original.address}</p>
      <div className="flex gap-2">
        <Badge className="bg-custom-neutral-100  py-[6px] px-2 rounded-lg">
          <p className="text-body text-black-500">{`${row.original.unit}/${row.original.totalUnits} listed`}</p>
        </Badge>
        <Badge className={`${bgColor}`}>
          <CircleIcon className={cn(`size-[7px] ${fillColor}`)} />
          {
            <span className={`text-body ${textColor}`}>
              {convertUnderscoreToSpace(row.original.status)}
            </span>
          }
        </Badge>
      </div>
